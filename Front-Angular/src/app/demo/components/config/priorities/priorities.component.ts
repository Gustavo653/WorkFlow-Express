...
export class PrioritiesComponent implements OnInit {
    ...
    constructor(protected layoutService: LayoutService, private priorityService: PriorityService, private confirmationService: ConfirmationService, private messageService: MessageService) {}
    ...
    getFormData(registry: any) {
        if (!registry) {
            ...
            if (registry.id) {
                this.priorityService.updatePriority(registry.id, registry.name).subscribe((x) => {
                    this.fetchData();
                    this.modalDialog = false;
                });
            } else {
                this.priorityService.createPriority(registry.name).subscribe((x) => {
                    this.fetchData();
                    this.modalDialog = false;
                });
            }
